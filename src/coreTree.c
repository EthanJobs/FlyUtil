...
TreeNode *Tree_insertNode_in(TreeNode *tn, TreeNode *newtn, int (*fun_compare)(void *, void *));
TreeNode *Tree_getValue_in(TreeNode *tn, void *value, int (*fun_compare)(void *, void *));
TreeNode *Tree_deleteValue_in(TreeNode *tn, void *value, int (*fun_compare)(void *, void *), void (*fun_free)(void *));
TreeNode *Tree_isHave_in(TreeNode *tn, void *value, int (*fun_compare)(void *, void *));
void Tree_free_in(TreeNode *tn, void (*fun_free)(void *));
...
void TreeNode_free(TreeNode *tn, void (*fun_free)(void *)) {
    if (tn == NULL) return;

    if (fun_free != NULL)
        fun_free((void *)tn->n_value);
    free(tn);
    tn = NULL;
...
Tree *Tree_init(int (*fun_compare)(void *, void *), void (*fun_free)(void *)) {
    if (fun_compare == NULL) return NULL;
    Tree *t = (Tree *)malloc(sizeof(Tree));

    t->n_treeNodeNum = 0;
    t->n_root = NULL;
    t->fun_compare = fun_compare;
    t->fun_free = fun_free;
...
TreeNode *Tree_insertNode_in(TreeNode *tn, TreeNode *newtn, int (*fun_compare)(void *, void *)) {
    ...
    int cmpTmp = fun_compare((void *)newtn->n_value, (void *)tn->n_value);

    if (cmpTmp < 0)
        tn->n_lChild = Tree_insertNode_in(tn->n_lChild, newtn, fun_compare);
    else if (cmpTmp > 0)
        tn->n_rChild = Tree_insertNode_in(tn->n_rChild, newtn, fun_compare);
    else
        return tn;
...
int Tree_insertNode(Tree *t, TreeNode *newtn) {
    ...
    t->n_root = Tree_insertNode_in(t->n_root, newtn, t->fun_compare);
    t->n_treeNodeNum++;
    return 0;
}
...
TreeNode *Tree_deleteValue_in(TreeNode *tn, void *value, int (*fun_compare)(void *, void *), void (*fun_free)(void *)) {
    int cmpTmp = fun_compare((void *)value, (void *)tn->n_value);

    if (cmpTmp < 0)
        tn->n_lChild = Tree_deleteValue_in(tn->n_lChild, value, fun_compare, fun_free);
    else if (cmpTmp > 0)
        tn->n_rChild = Tree_deleteValue_in(tn->n_rChild, value, fun_compare, fun_free);
    else {
        if (tn->n_lChild == NULL && tn->n_rChild == NULL) {
            TreeNode_free(tn, fun_free);
            return NULL;
        }
            ...
            tn->n_lChild = Tree_deleteValue_in(tn->n_lChild, tmpValue, fun_compare, NULL);
            if (fun_free) fun_free(tn->n_value);
            tn->n_value = tmpValue;
        }
    }
...
int Tree_deleteValue(Tree *t, void *value, int (*fun_compare)(void *, void *)) {
    if (t == NULL || value == NULL) return -1;

    if (Tree_isHave(t, value, fun_compare) == 1) {
        t->n_root = Tree_deleteValue_in(t->n_root, value, fun_compare, t->fun_free);
        t->n_treeNodeNum--;
        return 0;
    }
...
TreeNode *Tree_isHave_in(TreeNode *tn, void *value, int (*fun_compare)(void *, void *)) {
    ...
    if (cmpTmp < 0)
        return Tree_isHave_in(tn->n_lChild, value, fun_compare);
    else if (cmpTmp > 0)
        return Tree_isHave_in(tn->n_rChild, value, fun_compare);
    else
        return tn;
}

int Tree_isHave(Tree *t, void *value, int (*fun_compare)(void *, void *)) {
    if (Tree_isHave_in(t->n_root, value, fun_compare) != NULL) return 1;
    return 0;
}
...
TreeNode *Tree_getValue_in(TreeNode *tn, void *value, int (*fun_compare)(void *, void *)) {
    ...
    int cmpTmp = fun_compare((void *)value, (void *)tn->n_value);
    if (cmpTmp < 0)
        return Tree_getValue_in(tn->n_lChild, value, fun_compare);
    else if (cmpTmp > 0)
        return Tree_getValue_in(tn->n_rChild, value, fun_compare);
    else
        return tn;
}

void *Tree_getValue(Tree *t, void *value, int (*fun_compare)(void *, void *)) {
    if (t == NULL || t->n_root == NULL || value == NULL) return NULL;

    TreeNode *rettn = Tree_getValue_in(t->n_root, value, fun_compare);
    if (rettn != NULL)
...
void Tree_free_in(TreeNode *tn, void (*fun_free)(void *)) {
    ...
    Tree_free_in(tn->n_lChild, fun_free);
    Tree_free_in(tn->n_rChild, fun_free);
    if (fun_free != NULL)
        TreeNode_free(tn, fun_free);
...
void Tree_free(Tree *t) {
    ...
    Tree_free_in(t->n_root, t->fun_free);
    free(t);
    t = NULL;
...
int Tree_stackIterator_remove(Iterator *i) {
    ...
    Tree_deleteValue(((Tree_Iterator_StackData *)i->n_data)->t, ((Tree_Iterator_StackData *)i->n_data)->lasttn, ((Tree_Iterator_StackData *)i->n_data)->t->fun_compare);
...
int Tree_levelorderIterator_remove(Iterator *i) {
    ...
    Tree_deleteValue(((Tree_Iterator_QueueData *)i->n_data)->t, ((Tree_Iterator_QueueData *)i->n_data)->lasttn, ((Tree_Iterator_StackData *)i->n_data)->t->fun_compare);
